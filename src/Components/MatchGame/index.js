...
import ThumbnailLoader from '../ThumbnailLoder/index'
...
class MatchGame extends Component {
  state = {
    defaultTab: tabsList[0].tabId,
    randomImgObj: imagesList[0],
    result: false,
    score: 0,
    seconds: 60,
  }
  ...
  getScoreResult = imgId => {
    const {randomImgObj} = this.state
    const {id} = randomImgObj
    if (id === imgId) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        randomImgObj: imagesList[Math.floor(Math.random() * imagesList.length)],
      }))
    } else {
  ...
  render() {
    const {defaultTab, randomImgObj, score, result, seconds} = this.state

    const filteredData = imagesList.filter(
      eachItem => eachItem.category === defaultTab,
    )

    const gamePlay = (
      <div className="game-cont">
        <img src={randomImgObj.imageUrl} alt="match" className="random-img" />
        <ul className="tabs-cont">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isActive={defaultTab === eachTab.tabId}
            />
          ))}
        </ul>

        <ul className="thumbnail-cont">
          {filteredData.map(eachThumb => (
            <ThumbnailLoader
              key={eachThumb.id}
              thumbnail={eachThumb}
              getScoreResult={this.getScoreResult}
            />
          ))}
        </ul>
      </div>
    ...
    const myApp = (
      <div className="home-app-cont">
        ...
        {result ? (
          <GameResult finalScore={score} gameOn={this.gameOn} />
        ) : (
          gamePlay
        )}
      </div>
